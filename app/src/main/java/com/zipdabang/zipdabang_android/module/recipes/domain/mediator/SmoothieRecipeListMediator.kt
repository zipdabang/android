...
import com.zipdabang.zipdabang_android.module.recipes.mapper.toRecipeSmoothieEntity
import kotlinx.coroutines.flow.first

@OptIn(ExperimentalPagingApi::class)
class SmoothieRecipeListMediator(
    private val recipeApi: RecipeApi,
    private val database: RecipeDatabase,
    private val datastore: DataStore<Token>,
    private val categoryId: Int = 5,
    private val orderBy: String,
    private val isNetworkAvailable: Boolean
): RemoteMediator<Int, RecipeSmoothieEntity>() {

    private val recipeListDao = database.recipeSmoothieDao()
    private val remoteKeyDao = database.recipeRemoteKeyDao()

    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, RecipeSmoothieEntity>
    ): MediatorResult {
        return try {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, RecipeSmoothieEntity>): RemoteKeysEntity? {
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, RecipeSmoothieEntity>): RemoteKeysEntity? {
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, RecipeSmoothieEntity>): RemoteKeysEntity? {
