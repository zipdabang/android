...
import com.zipdabang.zipdabang_android.module.recipes.mapper.toRecipeNonCaffeineEntity
import kotlinx.coroutines.flow.first

@OptIn(ExperimentalPagingApi::class)
class NonCaffeineRecipeListMediator(
    private val recipeApi: RecipeApi,
    private val database: RecipeDatabase,
    private val datastore: DataStore<Token>,
    private val categoryId: Int = 2,
    private val orderBy: String,
    private val isNetworkAvailable: Boolean
): RemoteMediator<Int, RecipeNonCaffeineEntity>() {

    private val recipeListDao = database.recipeNonCaffeineDao()
    private val remoteKeyDao = database.recipeRemoteKeyDao()

    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, RecipeNonCaffeineEntity>
    ): MediatorResult {
        return try {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, RecipeNonCaffeineEntity>): RemoteKeysEntity? {
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, RecipeNonCaffeineEntity>): RemoteKeysEntity? {
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, RecipeNonCaffeineEntity>): RemoteKeysEntity? {
