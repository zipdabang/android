...
import com.zipdabang.zipdabang_android.module.recipes.mapper.toRecipeTeaEntity
import kotlinx.coroutines.flow.first

@OptIn(ExperimentalPagingApi::class)
class TeaRecipeListMediator(
    private val recipeApi: RecipeApi,
    private val database: RecipeDatabase,
    private val datastore: DataStore<Token>,
    private val categoryId: Int = 3,
    private val orderBy: String,
    private val isNetworkAvailable: Boolean
): RemoteMediator<Int, RecipeTeaEntity>() {

    private val recipeListDao = database.recipeTeaDao()
    private val remoteKeyDao = database.recipeRemoteKeyDao()

    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, RecipeTeaEntity>
    ): MediatorResult {
        return try {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, RecipeTeaEntity>): RemoteKeysEntity? {
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, RecipeTeaEntity>): RemoteKeysEntity? {
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, RecipeTeaEntity>): RemoteKeysEntity? {
