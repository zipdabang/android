...
class GeOtherRecipePreviewUseCase @Inject constructor(
    private val repository : MyApi,
    private val dataStore : DataStore<Token>
) {
    operator fun invoke(targetId : Int) : Flow<Resource<OtherRecipePreviewDto>> = flow{
        try{
            emit(Resource.Loading())
            val token = "Bearer ${dataStore.data.first().accessToken}"
            val followOrCancelResponse = repository.getOtherPreviewRecipe(accessToken = token, memeberId = targetId)
            emit(
                Resource.Success(
